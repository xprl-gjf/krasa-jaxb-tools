...
public class ValidSEIGenerator extends SEIGenerator {
    ...
    public void generate(ToolContext penv) throws ToolException {
        ...
        Map<QName, JavaModel> map = CastUtils.cast((Map<?, ?>) penv.get(
                WSDLToJavaProcessor.MODEL_MAP));
    ...
    private void parseArguments(ToolContext penv) {
        if (penv.get(ToolConstants.CFG_XJC_ARGS) != null) {
            ...
            for (String arg : xjcArgs) {
                ...
                if (parts[0].
                        contains(JaxbValidationsPlugins.GENERATE_SERVICE_VALIDATION_ANNOTATIONS)) {
                    parseValidationPolicy(parts[1]);
                }
                LOG.log(Level.FINE, "xjc arg:{0}", arg);
            }
        }
    }
